...
class PresencaSemanalService {
  ...
  async salvarStatus(ano: number, mes: number, semana: number, status: StatusSemana | null, turmaId?: string, motivo?: string): Promise<void> {
    const lista = this.carregarStatusStorage().filter(
      (r) => !(r.ano === ano && r.mes === mes && r.semana === semana && r.turmaId === turmaId)
    );
    if (status) {
      lista.push({ id: crypto.randomUUID(), ano, mes, semana, status, turmaId, motivo });
    }
